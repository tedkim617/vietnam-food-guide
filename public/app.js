...
function render(){
  const featured = state.data.foods.filter(f=>f.featured);
  const foods = state.data.foods;
  const content = `<div class="sectionTitle"><div><h2>${state.q||state.category?'검색 결과':'처음 먹기 좋은 추천 음식'}</h2><p>${foods.length}개 항목</p></div></div>
    ${!state.q && !state.category && featured.length ? `<div class="grid">${featured.map(foodCard).join('')}</div>`:''}
    <div class="sectionTitle"><div><h2>${state.category ? esc(catName(state.category)) : '전체 음식'}</h2><p>사진을 누르지 말고 “주문 정보 보기”로 크게 보여주세요.</p></div></div>
